    ...
    btnCast: function btnCast() {
      ...
      if (this.type == "Trigram") {
        ...
        axios.get('api/user/trigram?api_token=' + window.token).then(function (response) {
          _this.results = response.data.tri.code.split("");
          _this.name = response.data.tri.name;

          if (response.data.user == null) {
            _this.subscriber = false;
          }

          _this.trigram = response.data.tri;
          _this.isLoading = false;
          _this.isCast = true;
        });
      }
          ...
          directives: [
            {
              ...
              value:
                ...
                _vm.type != "Trigram",
              expression:
                "subscriber && isCast && !isLoading && type != 'Trigram'"
            }
          ],
        ...
        directives: [
          {
            ...
            rawName: "v-show",
            value:
              _vm.type == "Trigram" && _vm.isCast == true && _vm.subscriber,
            expression: "type == 'Trigram' && isCast == true && subscriber"
          }
        ]
      },
      [
        ...
        directives: [
          {
            ...
            value:
              ...
              _vm.subscriber,
            expression:
              "isCast && !isLoading && type != 'Trigram' && subscriber"
          }
        ]
      },
      [
