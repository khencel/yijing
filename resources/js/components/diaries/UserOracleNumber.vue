<template>
    <div class=" border">
        <h3 class="ml-2">Oracle Number</h3>
        <div v-if="Object.keys(oracles).length == 0" class="w-100 ml-2" style="color:gray">
            No record...
        </div>
        <div v-else class="mt-2 shadow p-3 background-diary-image" v-for="oracle in oracles" :key="oracle.id" >
            ...
            <div>
               <a class="btn btn-success" :href="'print?id='+oracle.id+'&type=Oracle'" target="_blank">Print PDF or Download</a>
            </div>
        </div>
    </div>
</template>
...
<script>
    export default {
        ...
        methods:{
            showOracleNumber(){
                axios.get('/api/oracle/show?api_token='+window.token)
                .then(res => {
